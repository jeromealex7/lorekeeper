...
class Timer(QtCore.QTimer):
    ...
    def __init__(self):
        ...
        self.start(1000)
        SIGNALS.TIMER_STATE.connect(self.set_state)
        SIGNALS.TIMER_RESET.connect(self.timer_reset)
    ...
    def execute(self):
        ...
        if message_box.clickedButton() == button_restart:
            SIGNALS.TIMER_RESET.emit(self.starting_time)
            SIGNALS.TIMER_STATE.emit(True)
    ...
    def step_seconds(self):
        ...
        if self.remaining_time >= 0:
            SIGNALS.TIMER_STEP.emit(self.remaining_time)
            if not self.remaining_time:
                SIGNALS.TIMER_STATE.emit(False)
                self.execute()
        else:
